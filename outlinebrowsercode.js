...
function slideUp (elem, duration, callback) { //replaces $(elem).slideUp
	if (elem === null) {
	...
	var saved = {
		height: elem.style.height, overflow: elem.style.overflow, transition: elem.style.transition,
		paddingTop: elem.style.paddingTop, paddingBottom: elem.style.paddingBottom,
		marginTop: elem.style.marginTop, marginBottom: elem.style.marginBottom
		};
	elem.style.height = elem.offsetHeight + "px";
	elem.style.overflow = "hidden";
	void elem.offsetHeight; //force a reflow so the transition has a starting value
	elem.style.transition = "height " + duration + "ms ease, padding " + duration + "ms ease, margin " + duration + "ms ease";
	elem.style.height = "0px";
	elem.style.paddingTop = "0px"; elem.style.paddingBottom = "0px";
	elem.style.marginTop = "0px"; elem.style.marginBottom = "0px";
	window.setTimeout (function () {
		elem.style.display = "none";
		elem.style.height = saved.height; elem.style.overflow = saved.overflow; elem.style.transition = saved.transition;
		elem.style.paddingTop = saved.paddingTop; elem.style.paddingBottom = saved.paddingBottom;
		elem.style.marginTop = saved.marginTop; elem.style.marginBottom = saved.marginBottom;
		if (callback !== undefined) {
			callback ();
			}
		}, duration);
	}
function slideDown (elem, duration, callback) { //replaces $(elem).slideDown
	if (elem === null) {
		if (callback !== undefined) {
			callback ();
			}
	...
	elem.style.display = "block"; //what the outline levels use
	var fullHeight = elem.offsetHeight;
	elem.style.overflow = "hidden";
	elem.style.height = "0px";
	elem.style.paddingTop = "0px"; elem.style.paddingBottom = "0px";
	elem.style.marginTop = "0px"; elem.style.marginBottom = "0px";
	void elem.offsetHeight; //force a reflow
	elem.style.transition = "height " + duration + "ms ease, padding " + duration + "ms ease, margin " + duration + "ms ease";
...
function getExpansionState () {
	...
	Array.prototype.forEach.call (theWedges, function (theWedge) {
		var flExpanded = theWedge.classList.contains ("fa-caret-down");
		if (flExpanded) {
			var id = theWedge.id; //something like idOutlineWedge17
...
function applyExpansionState (theList) {
	...
	for (var i = 0; i < splits.length; i++) {
		...
		var theLevel = document.getElementById ("idOutlineLevel" + splits [i]);
		if (theWedge !== null) {
			theWedge.setAttribute ("class", "fa fa-caret-down");
			theWedge.style.color = "silver";
			}
		if (theLevel !== null) {
...
function ecOutline (idnum) {
	var c = document.getElementById ("idOutlineWedge" + idnum), theUL = document.getElementById ("idOutlineLevel" + idnum);
