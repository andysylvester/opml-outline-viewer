//no-jQuery version -- the XML layer uses DOMParser and the DOM instead of
//jQuery, and readOutline reads over HTTP with fetch. The string utilities at
//the bottom used to come from scripting.com's basic code.js, which can't be
//loaded without jQuery, so they're defined here if nothing else defined them.
...
function xmlCompile (xmltext) { //3/27/17 by DW
	var theDoc = new DOMParser ().parseFromString (xmltext, "application/xml");
	var errors = theDoc.getElementsByTagName ("parsererror");
	if (errors.length > 0) {
...
function xmlGetAttribute (adrx, name) {
	if ((adrx === undefined) || (adrx === null) || (!adrx.hasAttribute (name))) {
		return (undefined); //same as jQuery's .attr for a missing attribute
		}
	return (adrx.getAttribute (name));
	}
function xmlGetAddress (adrx, name) {
	var theElements = adrx.getElementsByTagName (name);
...
function xmlGetSubValues (adrx) { //10/12/16 by DW
	...
	if ((adrx === undefined) || (adrx === null)) {
		return (values);
		}
	Array.prototype.forEach.call (adrx.children, function (theChild) {
		var name = xmlGetNodeNameProp (theChild);
		if (name.length > 0) {
			var val = theChild.textContent;
...
function xmlGetNodeNameProp (adrx) { //12/10/13 by DW
	return (adrx.nodeName);
	}
function xmlHasSubs (adrx) {
	return (adrx.children.length > 0);
	};
...
function outlineToJson (adrx, nameOutlineElement) { //12/25/20 by DW
	...
	if (xmlHasSubs (adrx)) {
		theOutline.subs = [];
		Array.prototype.forEach.call (adrx.children, function (theChild) {
			if (theChild.nodeName.toLowerCase () == nameOutlineElement.toLowerCase ()) {
				theOutline.subs [theOutline.subs.length] = outlineToJson (theChild, nameOutlineElement);
				}
			});
		}
	return (theOutline);
...
function readOutline (urlOpmlFile, options, callback) { //9/24/21 by DW
	...
	function readHttpFile (url, timeoutInMilliseconds, headers, callback) {
		...
		fetch (url, {headers: headers, signal: theController.signal})
...
//string utilities that used to come from scripting.com's basic code.js, which
//depends on jQuery. They're only defined if some other script hasn't already
//defined them. Used by opmlStringify, getOutlineHtml, markdownToOutline,
//outlineToMarkdown and readOutline.

if (typeof stringLower === "undefined") {
	window.stringLower = function (s) {
		if (s === undefined) {
			return ("");
			}
		s = s.toString ();
...
if (typeof stringDelete === "undefined") {
	window.stringDelete = function (s, ix, ct) {
		var start = ix - 1;
		var end = (ix + ct) - 1;
...
if (typeof stringContains === "undefined") {
	window.stringContains = function (s, whatItMightContain, flUnicase) {
		if (s === undefined) {
			return (false);
			}
		if (flUnicase === undefined) {
...
if (typeof beginsWith === "undefined") {
	window.beginsWith = function (s, possibleBeginning, flUnicase) {
...
if (typeof filledString === "undefined") {
	window.filledString = function (ch, ct) {
		var s = "";
		for (var i = 0; i < ct; i++) {
			s += ch;
			}
...
if (typeof encodeXml === "undefined") {
	window.encodeXml = function (s) {
		if (s === undefined) {
			return ("");
			}
		var charMap = {
		...
		s = s.toString ();
		s = s.replace (new RegExp (String.fromCharCode (160), "g"), " ");
